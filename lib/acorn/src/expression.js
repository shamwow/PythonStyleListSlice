...
pp.parseComputedMemberRangeExpression = function(noIn, refShorthandDefaultPos) {
  let start, end, skip;

  if (this.type !== tt.colon) {
    start = this.parseExpression(noIn, refShorthandDefaultPos);
  }

  if (this.eat(tt.colon)) {
    if (this.type === tt.num) {
      end = this.parseExpression(noIn, refShorthandDefaultPos);
    }

    if (this.eat(tt.colon)) {
      if (this.type === tt.num) {
        skip = this.parseExpression(noIn, refShorthandDefaultPos);
      }
    }
  }

  if (!start || end || skip) {
    let node = this.startNodeAt(this.startPos, this.startLoc);
    node.startExpression = start;
    node.endExpression = end;
    node.skipExpression = skip;

    return this.finishNode(node, 'ComputedMemberRangeExpression');
  }
...
pp.parseMaybeAssign = function(noIn, refShorthandDefaultPos, afterLeftParse) {
  ...
  if (this.type.isAssign) {
    ...
    if (left.type === 'MemberExpression' && left.computed) {
      return this.finishNode(node, "ComputedMemberAssignmentExpression")
    }
    // END MODIFIED
    else {
...
pp.parseSubscripts = function(base, startPos, startLoc, noCalls) {
  for (;;) {
    if (this.eat(tt.dot)) {
      ...
      node.object = base
      // MODIFIED
      node.property = this.parseComputedMemberRangeExpression()
      // END MODIFIED
      node.computed = true
